<script lang="ts" setup>

</script>

<template>
    <div class="max-w-lg px-4 mx-auto text-left md:max-w-none md:text-center font-main py-10">

        <h1 class="text-4xl font-bold">One app for the full guest experience!</h1>


        <div class="mt-8">

            <div
                class="px-4 mx-auto sm:max-w-xl md:max-w-full lg:max-w-screen-xl md:px-24 lg:px-8 sm:border-0 rounded-xl sm:rounded-0 border">
                <div class="flex flex-col items-center justify-between w-full lg:flex-row">
                    <img class="w-[180px] sm:w-[240px] lg:w-[320px]" alt="logo" src="/img/localife_web.png" />
                    <div class="lg:mb-0 lg:max-w-lg lg:pr-5">
                        <div class="max-w-xl my-8 text-center lg:text-left">
                            <h2
                                class="text-2xl sm:mt-0 mt-6 font-semibold tracking-tight sm:text-3xl sm:leading-none max-w-lg mb-6">
                                Explore individually
                            </h2>
                            <p class="text-lg">Get personalized suggestions for single points of interest based on your
                                preferences</p>
                        </div>
                    </div>
                </div>
            </div>

            <div
                class="px-4 mx-auto sm:max-w-xl md:max-w-full lg:max-w-screen-xl md:px-24 lg:px-12 sm:border-0 rounded-xl sm:rounded-0 border sm:mt-0 mt-8">
                <div class="flex flex-col items-center justify-between w-full lg:flex-row">
                    <div class="lg:mb-0 lg:max-w-lg lg:pr-5">
                        <div class="max-w-xl my-8 text-center lg:text-left">
                            <h2
                                class="text-2xl font-semibold tracking-tight sm:pt-0 pt-5 sm:text-3xl sm:leading-none max-w-lg mb-6">
                                Follow thematic tours
                            </h2>
                            <p class="text-lg">Get yourself your own personal guide that will show you around and tell
                                you
                                thematic stories based on a range of different places.</p>
                        </div>
                    </div>
                    <img class="w-[180px] sm:w-[240px] lg:w-[320px]" alt="logo" src="/img/tours_web.png" />
                </div>
            </div>

        </div>

        <div class="sm:mt-20 mt-8">

            <div
                class="px-4 mx-auto sm:max-w-xl md:max-w-full lg:max-w-screen-xl md:px-24 lg:px-8 sm:border-0 rounded-xl sm:rounded-0 border">
                <div class="flex flex-col items-center justify-between w-full lg:flex-row">
                    <img class="w-[180px] sm:w-[240px] lg:w-[320px]" alt="logo" src="/img/modes_web.png" />
                    <div class="lg:mb-0 lg:max-w-lg lg:pr-5">
                        <div class="max-w-2xl my-8 text-center lg:text-left">
                            <h2
                                class="text-2xl sm:mt-0 mt-6 font-semibold tracking-tight sm:text-3xl sm:leading-none max-w-lg mb-6">
                                Get informed about events and other special offers
                            </h2>
                            <p class="text-lg">Check the events section to know about all upcoming events
                                around you.
                                Interested?
                                The app will guide you there!</p>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>

</template>

<style lang="postcss" scoped></style>
